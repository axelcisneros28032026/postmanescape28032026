...
class Level1(QWidget):
    ...
    def __init__(self, rol="player", host=False, ip="127.0.0.1", port=25565):
        ...
        self.round = 1 # Número de ronda
        self.party_status = True # Estado de partida actualT
...
# ======================================================================================================================
    ...
    def _net_send(self, data: dict):
        """Envía un paquete al peer (servidor o cliente)."""
        if not self._net_ready:
            return
        if self.rol == "enemy" and hasattr(self, "_server"):
            self._server.send(data)
        elif self.rol == "player" and hasattr(self, "_client"):
    ...
    def _net_send_state(self):
        """Envía el estado local al peer en cada tick de red."""
        if self.rol == "enemy":
            packet = {
                "type": "state",
                "enemy": {
    ...
    def _net_on_received(self, packet: dict):
        ...
        if ptype == "state":
            ...
            self.manage_game_progress(winner=winner)
